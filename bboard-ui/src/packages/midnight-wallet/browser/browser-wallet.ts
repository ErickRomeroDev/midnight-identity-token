...
import { checkProofServerStatus } from '@/packages/midnight-core';
...
export class MidnightBrowserWallet {
  ...
  async balanceAndProveTransaction(tx: Transaction, newCoins: CoinInfo[]): Promise<Transaction | undefined> {
  ...
  async submitTransaction(tx: Transaction): Promise<string | undefined> {
    if (this._walletInstance) {
