...
export class Simulator {
  ...
  static deployContract(secretKey: Uint8Array): Simulator {
    return new Simulator(createPrivateState(secretKey));
  }
  ...
  coin(value: number): CoinInfo {
    return {
      nonce: randomSk(),
      color: encodeTokenType(tokenType(pad('test_coin', 32), this.contractAddress)),
      value: BigInt(value),
    };
  }  

  post(): Ledger {
    return this.updateStateAndGetLedger(this.contract.impureCircuits.post(this.turnContext, 'I will bet'));
  }

  take_down(): Ledger {
    const circuitResults = this.contract.impureCircuits.take_down(this.turnContext);
    return this.updateStateAndGetLedger(circuitResults);
  }

  charge(value: number): Ledger {
    const circuitResults = this.contract.impureCircuits.charge(this.turnContext, this.coin(value));
    return this.updateStateAndGetLedger(circuitResults);
  }

  withdraw(): Ledger {
    const circuitResults = this.contract.impureCircuits.withdraw(this.turnContext);
    return this.updateStateAndGetLedger(circuitResults);
  }
}
