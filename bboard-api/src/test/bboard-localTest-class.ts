...
import { type Contract, type Witnesses } from '@meshsdk/token-contract';
...
interface CoinPrivateState {}
type CoinPrivateStates = Record<string, CoinPrivateState>;
export type CoinContract = Contract<CoinPrivateState, Witnesses<CoinPrivateState>>;
type CoinCircuitKeys = Exclude<keyof CoinContract['impureCircuits'], number | symbol>;
export type CoinProviders = MidnightProviders<CoinCircuitKeys, CoinPrivateStates>;
export type DeployedBrickTowersCoin = FoundContract<CoinPrivateState, CoinContract>;
  ...
  start = async (): Promise<TestConfiguration> => {
    if (process.env.RUN_STANDALONE !== 'true') {
      ...
      this.dockerEnv = new DockerComposeEnvironment(path.resolve(currentDir, '..', '..'), composeFile)
        ...
        .withWaitStrategy('api-node', Wait.forLogMessage(/Running JSON-RPC server/, 1));
      this.env = await this.dockerEnv.up();

      this.testConfig.dappConfig = {
        ...this.testConfig.dappConfig,
        indexer: TestEnvironment.mapContainerPort(this.env, this.testConfig.dappConfig.indexer, 'api-indexer'),
        indexerWS: TestEnvironment.mapContainerPort(
          this.env,
          this.testConfig.dappConfig.indexerWS,
          'api-indexer',
        ),
        node: TestEnvironment.mapContainerPort(this.env, this.testConfig.dappConfig.node, 'api-node'),
...
export class TestProviders {
  ...
  configureProviders = async (wallet: Wallet & Resource, config: Config) => {
    ...
    const inMemory = inMemoryPrivateStateProvider<PrivateStates>();
    return {
      privateStateProvider: inMemory,
      publicDataProvider: indexerPublicDataProvider(config.indexer, config.indexerWS),
      zkConfigProvider: new NodeZkConfigProvider<'post' | 'take_down' | 'charge' | 'withdraw'>(
        config.bboardZkConfigPath,
      ),
  ...
  configureTokenProviders = async (wallet: Wallet & Resource, config: Config) => {
    const walletAndMidnightProvider = await this.createWalletAndMidnightProvider(wallet);
    const inMemory = inMemoryPrivateStateProvider<CoinPrivateStates>();
    return {
      privateStateProvider: inMemory,
      publicDataProvider: indexerPublicDataProvider(config.indexer, config.indexerWS),
      zkConfigProvider: new NodeZkConfigProvider<'mint' | 'owner_withdraw'>(config.tokenZkConfigPath),
      proofProvider: httpClientProofProvider(config.proofServer),
      walletProvider: walletAndMidnightProvider,
      midnightProvider: walletAndMidnightProvider,
    };
